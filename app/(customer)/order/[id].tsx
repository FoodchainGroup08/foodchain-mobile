...
const STATUS_LABEL: Record<string, string> = { RECEIVED: 'Order Received', CONFIRMED: 'Confirmed', PREPARING: 'Preparing', READY: 'Ready', PICKED_UP: 'Picked Up', SERVED: 'Served', COMPLETED: 'Completed', CANCELLED: 'Cancelled' };
const STATUS_EMOJI: Record<string, string> = { RECEIVED: '📥', CONFIRMED: '✅', PREPARING: '👨‍🍳', READY: '🍱', PICKED_UP: '🛵', SERVED: '🍽️', COMPLETED: '🎉', CANCELLED: '❌' };

function statusColor(status: string): string {
  switch (status) {
    case 'RECEIVED': return Colors.statusReceived;
    case 'CONFIRMED': case 'PREPARING': return Colors.statusPreparing;
    case 'READY': case 'PICKED_UP': case 'SERVED': return Colors.statusReady;
  ...
  const handleCancel = () => {
    Alert.alert('Cancel Order', 'Are you sure you want to cancel this order?', [
      ...
      { text: 'Cancel Order', style: 'destructive', onPress: async () => {
        try {
          await cancelOrder(id!, undefined, 'Customer cancelled');
    ...
    <SafeAreaView style={styles.flex} edges={['top']}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.container}>
        ...
        <View style={[styles.heroCard, { borderTopColor: statusColor(order.status) }]}>
          ...
          <Text style={[styles.heroStatus, { color: statusColor(order.status) }]}>{STATUS_LABEL[order.status] ?? order.status}</Text>
          <Text style={styles.heroOrderId}>Order #{order.id.slice(-8).toUpperCase()}</Text>
          {order.estimatedTime && !isFinal && (
            <View style={styles.etaBadge}><Text style={styles.etaText}>⏱ {order.estimatedTime}</Text></View>
          )}
        </View>
            ...
            {STATUS_STEPS.map((step, i) => {
              ...
              return (
                <View key={step}>
                  ...
                  {i < STATUS_STEPS.length - 1 && <View style={[styles.stepConnector, i < currentIdx && styles.stepConnectorDone]} />}
                </View>
              );
            })}
          </View>
        ...
        <View style={styles.infoCard}>
          ...
          {[
            ...
            <View key={i} style={styles.infoRow}>
              ...
              <Text style={[styles.infoValue, (r as any).capitalize && { textTransform: 'capitalize' }]}>{r!.value}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.itemsCard}>
          ...
          <View style={styles.itemRow}>
            ...
            <Text style={[styles.itemPrice, { color: Colors.amber, fontFamily: Fonts.bold, fontSize: 16 }]}>₦{order.total.toLocaleString()}</Text>
          </View>
        </View>

        {canCancel && <TouchableOpacity style={styles.cancelButton} onPress={handleCancel}><Text style={styles.cancelButtonText}>Cancel Order</Text></TouchableOpacity>}
        <TouchableOpacity style={styles.refreshButton} onPress={() => qc.invalidateQueries({ queryKey: ['order', id] })}>
...
const styles = StyleSheet.create({
  ...
  heroCard: { backgroundColor: Colors.surface, borderRadius: Radius.xl, padding: 28, alignItems: 'center', borderWidth: 1, borderColor: Colors.border, borderTopWidth: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3 },
  ...
  stepsCard: { backgroundColor: Colors.surface, borderRadius: Radius.lg, padding: 20, borderWidth: 1, borderColor: Colors.border },
  stepRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  stepDot: { width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.muted, justifyContent: 'center', alignItems: 'center' },
  ...
  infoCard: { backgroundColor: Colors.surface, borderRadius: Radius.lg, padding: 16, borderWidth: 1, borderColor: Colors.border, gap: 10 },
  ...
  itemsCard: { backgroundColor: Colors.surface, borderRadius: Radius.lg, padding: 16, borderWidth: 1, borderColor: Colors.border, gap: 8 },
  ...
  cancelButton: { borderWidth: 1.5, borderColor: Colors.error, borderRadius: Radius.md, paddingVertical: 14, alignItems: 'center' },
  cancelButtonText: { color: Colors.error, fontSize: 14, fontFamily: Fonts.semiBold },
  refreshButton: { borderWidth: 1.5, borderColor: Colors.border, borderRadius: Radius.md, paddingVertical: 14, alignItems: 'center' },
